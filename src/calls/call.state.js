...
class CallStateManager {
  ...
  async initiateCall({ caller, receiverId, callType, io }) {
    ...
    if (isOnline && receiverSockets.length > 0) {
      ...
      console.log(`[CALL] Invitation delivered`);
    } else {
      console.log(`[CALL] Receiver BACKGROUND/OFFLINE`);
      const sent = await fcmService.sendIncomingCallMessage(receiver.fcmToken, callSession);
      if (sent) {
        console.log(`[CALL] FCM dispatched`);
      }
    }
