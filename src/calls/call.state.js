...
class CallStateManager {
  ...
  async initiateCall({ caller, receiverId, callType, requestedCallId, io }) {
    ...
    if (receiver.fcmToken) {
      console.log(`[CALL] Dispatching FCM notification to receiver...`);
      fcmService.sendIncomingCallMessage(receiver.fcmToken, callSession).catch(err => {
        console.error(`[CALL] FCM dispatch failed:`, err.message);
      });
    }
