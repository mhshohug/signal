  ...
  io.on('connection', (socket) => {
    ...
    socket.on('call-user', async (data, callback) => {
      try {
        ...
        const { receiverId, callType, callId, call_id } = data; // callType: 'AUDIO' | 'VIDEO'
        const requestedCallId = callId || call_id;
    ...
    socket.on('offer', (data) => {
      const { callId, call_id, sdp, targetUid } = data || {};
      const effectiveCallId = callId || call_id;
      console.log(`[WEBRTC] Offer received call_id=${effectiveCallId}`);

      if (effectiveCallId) {
        const call = db.getCall(effectiveCallId);
        if (call) {
          db.updateCallStatus(effectiveCallId, 'WEBRTC_CONNECTING');
        }
      }

      const targetSockets = db.getUserSocketIds(targetUid);
      targetSockets.forEach(sId => {
        io.to(sId).emit('offer', { callId: effectiveCallId, call_id: effectiveCallId, sdp, senderUid: user.uid });
      });
    });
    ...
    socket.on('answer', (data) => {
      const { callId, call_id, sdp, targetUid } = data || {};
      const effectiveCallId = callId || call_id;
      console.log(`[WEBRTC] Answer received call_id=${effectiveCallId}`);
      ...
      const targetSockets = db.getUserSocketIds(targetUid);
      targetSockets.forEach(sId => {
        io.to(sId).emit('answer', { callId: effectiveCallId, call_id: effectiveCallId, sdp, senderUid: user.uid });
      });
    });

    // --- 5. WEBRTC SIGNALING: ICE CANDIDATE ---
    socket.on('ice-candidate', (data) => {
      const { callId, call_id, candidate, targetUid } = data || {};
      const effectiveCallId = callId || call_id;

      const targetSockets = db.getUserSocketIds(targetUid);
      targetSockets.forEach(sId => {
        io.to(sId).emit('ice-candidate', { callId: effectiveCallId, call_id: effectiveCallId, candidate, senderUid: user.uid });
      });
    });

    // --- 6. CALL CONNECTED NOTIFICATION ---
    socket.on('call-connected', ({ callId }) => {
      console.log(`[WEBRTC] Connection state: connected for call_id=${callId}`);
      console.log(`[CALL] Call connected for call_id=${callId}`);
      ...
      if (call) {
        ...
        [...callerSockets, ...receiverSockets].forEach(sId => {
          io.to(sId).emit('call-connected', { callId, call_id: callId, connectedAt: call.connectedAt });
        });
      }
    });
    ...
    socket.on('disconnect', (reason) => {
      ...
      // Clean up any active call involving this disconnected user
      const userCalls = db.getCallsForUser ? db.getCallsForUser(user.uid) : [];
      userCalls.forEach(userCall => {
        console.log(`[CALL] Ending call ${userCall.callId} due to user ${user.email} disconnect (${reason})`);
        callStateManager.endCall(userCall.callId, `USER_DISCONNECTED_${reason.toUpperCase()}`, io);
      });
