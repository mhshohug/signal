...
function setupSignalingSocket(io) {
  io.on('connection', (socket) => {
    ...
    socket.on('offer', (data) => {
      ...
      console.log(`[WEBRTC] Offer received for call_id=${callId}`);
      ...
      targetSockets.forEach(sId => {
        io.to(sId).emit('offer', { callId, call_id: callId, sdp, senderUid: user.uid });
      });
    });
    ...
    socket.on('answer', (data) => {
      const { callId, sdp, targetUid } = data;
      console.log(`[WEBRTC] Answer received for call_id=${callId}`);
      ...
      const targetSockets = db.getUserSocketIds(targetUid);
      targetSockets.forEach(sId => {
        io.to(sId).emit('answer', { callId, call_id: callId, sdp, senderUid: user.uid });
      });
    });
    ...
    socket.on('ice-candidate', (data) => {
      ...
      console.log(`[WEBRTC] ICE candidate received for call_id=${callId}`);

      const targetSockets = db.getUserSocketIds(targetUid);
      targetSockets.forEach(sId => {
        io.to(sId).emit('ice-candidate', { callId, call_id: callId, candidate, senderUid: user.uid });
      });
    });
